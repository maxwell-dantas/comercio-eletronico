        ...
        if not itens_carrinho:
            ...
            for item in itens_carrinho:
                with st.container(border=True):
                    ...
                    # Usa o preço já com desconto
                    preco_unitario = item.get("preco", 0.0)
                    preco_base_sem_desconto = produto_detalhes.get("preco", preco_unitario)
                    ...
                    with col_info:
                        ...
                        if preco_unitario < preco_base_sem_desconto:
                            st.caption(f"Qtd: {quantidade} x R$ {preco_unitario:.2f} (Promoção Aplicada) | Subtotal: R$ {subtotal:.2f}")
                        else:
                            st.caption(f"Qtd: {quantidade} x R$ {preco_unitario:.2f} | Subtotal: R$ {subtotal:.2f}")
