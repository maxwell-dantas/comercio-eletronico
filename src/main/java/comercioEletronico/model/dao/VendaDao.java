package comercioEletronico.model.dao;

import comercioEletronico.model.entities.Venda;

import java.util.ArrayList;
import java.lang.reflect.Type;
import java.io.IOException;
import java.io.FileWriter;
import java.io.FileReader;

import com.google.gson.Gson;
import com.google.gson.reflect.TypeToken;

public class VendaDao {
    private static ArrayList<Venda> listaVendas = new ArrayList<>();
    private static final Gson gson = new Gson();

    public void inserir(Venda venda) {
        abrir();
        int idGenerator = 1;
        if (!listaVendas.isEmpty()) {
            idGenerator = listaVendas.getLast().getId() + 1;
        }
        venda.setId(idGenerator);
        listaVendas.add(venda);
        salvar();
    }

    public Venda listarId(int id) {
        abrir();
        for (Venda venda : listaVendas) {
            if (venda.getId() == id) {
                return venda;
            }
        }
        return null;
    }

    // O parâmetro deve ser o ID, pois o objeto que vem da View possui uma referência de memória diferente dos objetos recém-criados pelo GSON ao ler o arquivo JSON.
    public void atualizar(int id, boolean carrinho, double total) {
        Venda venda = listarId(id);
        if (venda != null) {
            venda.setCarrinho(carrinho);
            venda.setTotal(total);
            salvar();
        }
    }

    // O parâmetro deve ser o ID, pois o objeto que vem da View possui uma referência de memória diferente dos objetos recém-criados pelo GSON ao ler o arquivo JSON.
    public void remover(int id) {
        Venda venda = listarId(id);
        if (venda != null) {
            listaVendas.remove(venda);
            salvar();
        }
    }

    public ArrayList<Venda> listar() {
        abrir();
        return listaVendas;
    }

    public void abrir() {
        try {
            // 1. Abre o leitor para o caminho onde o arquivo foi salvo
            FileReader leitor = new FileReader("data/vendas.json");

            // 2. Verifica o tipo da lista
            Type listaTipo = new TypeToken<ArrayList<Venda>>() {
            }.getType();

            listaVendas = gson.fromJson(leitor, listaTipo); // 3. Leitura de arquivo

            if (listaVendas == null) { // camada de segurança extra (caso o usuário não insira nada)
                listaVendas = new ArrayList<>();
            }

            leitor.close();  // 4. fecha o leitor

        } catch (IOException e) {
            // Caso o arquivo não exista ainda, cria uma lista vazia para evitar erros (o método gson.fromJson)
            listaVendas = new ArrayList<>();
        }
    }

    public void salvar() {
        try {
            // 1. Abertura de arquivo
            FileWriter escritor = new FileWriter("data/vendas.json");

            // 2. Converte a lista de objetos em texto Json
            String textoJson = gson.toJson(listaVendas);

            // 3. Escreve o texto no arquivo
            escritor.write(textoJson);

            // 4. Fecha o arquivo para confirmar a gravação
            escritor.close();

        } catch (IOException e) {
            System.out.println("Ocorreu um erro ao salvar o arquivo: " + e.getMessage());
        }
    }
}